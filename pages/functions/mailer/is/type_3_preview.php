...
if ($_REQUEST["type_fault"]!=NULL && $_REQUEST["type_receivers"]!=NULL)
{
    ...
    $search_element = $ems.' (SLOT '.$slot.')';
    
    $found_ems = '';
    
    $sql_ems = "SELECT * FROM faults WHERE element = '".$search_element."'";
    $result = $conn->query($sql_ems);
    
    if ($result->num_rows >0) {
        $found_ems .= 'Για το slot '.$slot.' του <strong>'.$ems.'</strong> έχουν σταλεί τα εξής mail:<br/>';
    ...
    $_top .= "<strong>ΠΙΘΑΝΟ ΠΡΟΒΛΗΜΑ DSLAM ".$search_element."</strong><br />";
    ...
    $_mainDesc = 'Eμφανίζεται πιθανό πρόβλημα στο <strong>slot '.$slot.'</strong> του D_'.$dslam.'<br />';
